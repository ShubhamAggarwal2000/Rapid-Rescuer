...
# Team ID:	2177		[ Team-ID ]
# Author List:	Gaurav , Nilendu ,Shubham ,Gukan	[ Names of team members worked on this file separated by Comma: Name1, Name2, ... ]
# Filename:			task_1a.py
...
def solveMaze(original_binary_img, initial_point, final_point, no_cells_height, no_cells_width):
    ...
    path = []
    solution = {}
    start_x = initial_point[0]
    start_y = initial_point[1]
    end_x = 2 * (final_point[0])
    end_y = 2 * (final_point[1])
    ...
    def search():
        while len(frontier) > 0:
            ...
            [x, y] = current
            if [x, y - 1] in path and [x, y - 1] not in visited:  # left
            ...
            # print("l")
            # print(frontier)
            if [x, y + 1] in path and [x, y + 1] not in visited:  # right
                frontier.append([x, y + 1])
                cell = (x, y + 1)
                solution[cell] = x, y
                visited.append([x, y + 1])
            # print("r")
            # print(frontier)
            if [x + 1, y] in path and [x + 1, y] not in visited:  # down
            ...
            # print("d")
            # print(frontier)
            if [x - 1, y] in path and [x - 1, y] not in visited:  # up
                frontier.append([x - 1, y])
                cell = (x - 1, y)
                solution[cell] = x, y
                visited.append([x - 1, y])
    ...
    search()
    backtrack()
    shortestPath.append((int(end_x / 2), int(end_y / 2)))
    for i in shortestPath1:
        if (i[0] + i[1]) % 2 == 0:
            j = (int(i[0] / 2), int(i[1] / 2))
...
def Maze_matrix(maze1, no_cells_height1, no_cells_width1):
    ...
    for i in range(0, (no_cells_width1)):
        for j in range(0, (no_cells_height1 - 1)):
            for m in range(20 * (j) + 10, 20 * (j + 1) + 10):
                if maze1[20 * (i) + 10][m] == 0:
                    matrix[2 * i][(2 * j) + 1] = 1
                    break

    for i in range(0, (no_cells_width1 - 1)):
        for j in range(0, (no_cells_height1)):
            for m in range(20 * (i) + 10, 20 * (i + 1) + 10):
                if maze1[m][20 * (j) + 10] == 0:
                    matrix[(2 * i) + 1][2 * j] = 1
                    break
    np.set_printoptions(threshold=np.inf)
    # print(matrix)
    ...
    if choice == 'y':
        ...
        for file_num in range(file_count):
            ...
            try:
                ...
                shortestPath = solveMaze(original_binary_img, initial_point, final_point, no_cells_height,
                                         no_cells_width)
